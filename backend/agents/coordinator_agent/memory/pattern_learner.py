...
def _llm_analyze_patterns(
    ...
    """
    Ask the LLM to analyze history and identify patterns.
    OPTIMIZED: Early exit if not enough history, reduced max_tokens, better prompting.
    """
    # ── OPTIMIZATION 1: Early exit if not enough history ─────────────────────
    if len(history_entries) < 3:
        logger.info(f"Not enough history for LLM analysis ({len(history_entries)} entries, need 3)")
        return []
    
    # ── OPTIMIZATION 2: Limit history length to last 10 entries ──────────────
    # This reduces token count while preserving recent patterns
    limited_history = history_entries[-10:] if len(history_entries) > 10 else history_entries
    
    # ── OPTIMIZATION 3: Deduplicate existing patterns to reduce prompt size ──
    unique_existing = list(set(existing_patterns))[:5]  # Only send top 5
    
    client = _get_llm_client()

    history_block = "\n".join(
        f"[{i+1}] {entry[:150]}" for i, entry in enumerate(limited_history)  # Truncate long entries
    )

    existing_block = (
        "\n".join(f"- {p}" for p in unique_existing)
        if unique_existing
        else "None yet."
    )

    prompt = f"""Analyze user behavior to find patterns.

RECENT HISTORY ({len(limited_history)} entries):
{history_block}
...
Rules:
- Only identify patterns that appear 2+ times
- Be specific
...
[
  {{
    "preference": "clear statement about user behavior",
    "category": "app_usage|contact|workflow|preference|account",
...
If no new patterns, return: []"""
    ...
    try:
        response = client.chat.completions.create(
            ...
            temperature=0.1,
            max_tokens=400  # REDUCED from 800 to 400 (saves 50% tokens)
        )
        ...
        logger.info(f"LLM identified {len(valid)} patterns from {len(limited_history)} entries")
        return valid
